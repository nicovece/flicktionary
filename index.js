...
let allowedOrigins = [
  ...
  "http://localhost:1234",
  "http://localhost:4200",
  "https://flicktionary.netlify.app",
  "https://nicovece.github.io",
];
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!movie) {
        return res
          .status(404)
          .send('Movie with title "' + Title + '" not found');
      }
      res.json(movie);
    } catch (err) {
      console.error(err);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!movie) {
        return res
          .status(404)
          .send('Director "' + directorName + '" not found');
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!movies || movies.length === 0) {
        return res
          .status(404)
          .send("No movies found matching your search criteria");
      }
...
app.put(
  ...
  [
    ...
    check("Email")
      .optional()
      .isEmail()
      .withMessage("Email does not have a valid format"),
    check("Password")
      .optional()
      .isLength({ min: 8 })
      .withMessage("Password must be at least 8 characters long")
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .send('User with username "' + Username + '" successfully removed.');
    } catch (err) {
      console.error(err);
      res.status(500).send("Error: " + err);
    }
